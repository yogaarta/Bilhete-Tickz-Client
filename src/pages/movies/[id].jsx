...
const MovieDetail = () => {
   ...
   const { id = '', location = '', sort = '', order = '', page = '' } = router.query;
   useEffect(() => {
                  ...
                  <div className="d-flex justify-content-between">
                     ...
                     <div className={`col-md-8 col-8 ${styles.descDetail}`}>
                        ...
                        <div className="mt-2 mt-md-4">
                           Release date
                           <section className="text-dark">{movies.release_date ? movies.release_date.slice(0, 10) : ''}</section>
                        </div>
                        <div className="mt-2 mt-md-4">
                           Duration
                        ...
                        <div className="mt-2 mt-md-4">
                           Directed by
                           <section className="text-dark">{movies.director ? movies.director : null}</section>
                        </div>
                        <div className="mt-2 mt-md-4">
                           Casts
                           <section className="text-dark">{movies.cast ? movies.cast : ''}</section>
                        </div>
                     </div>
                  </div>
                  <div className={`mt-5 ${styles.synopsis}`}>
                     Synopsis
                     <section className="mt-4">{movies.synopsis ? movies.synopsis : ''}</section>
                  </div>
               </>
            ) : (
            ...
            <div className={`${styles.showTimes}`}>
                        ...
                        <div className={`position-absolute mt-3 ${styles.dropContent}`}>
                           <div
                              onClick={(e) => {
                                 ...
                                 if (router.asPath.includes('date')) {
                                    router.push(`/movies/${router.query.id}?location=jakarta&date=${router.query.date}`);
                                 }
                                 router.push(`/movies/${router.query.id}?location=jakarta`);
                              }}
                              className="my-4"
                           >
                              Jakarta
                           ...
                           <div
                              onClick={(e) => {
                                 e.preventDefault();
                                 if (router.asPath.includes('date')) {
                                    router.push(`/movies/${router.query.id}?location=bandung&date=${router.query.date}`);
                                 }
                                 router.push(`/movies/${router.query.id}?location=bandung`);
                              }}
                              className="my-4"
                           >
                              Bandung
               ...
               <div className="d-flex justify-content-center gap-3 flex-wrap mt-5">
                  {cinemas.length > 0 ? (
                     <>
                        {cinemas.map((item) => (
                           <CardCinema key={item.id} address={item.address} pictures={item.pictures} time={item.list_time} location={item.location} price={item.price} name={item.name} />
                        ))}
                     </>
                  ) : (
                     <>
